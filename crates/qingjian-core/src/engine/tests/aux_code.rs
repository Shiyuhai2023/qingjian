...
fn aux_engine() -> Engine {
    engine().with_aux_codes(vec![codes()])
}
...
/// 没挂码表时：辅码态照常进，但任何码都筛掉一切（CLI / 测试里没装码表的零开销路径）。
#[test]
fn without_any_table_every_code_filters_everything() {
    let mut bare = aux_engine();
    bare.set_aux_codes(Vec::new());
    bare.set_input("kaifa");
    bare.enter_aux();
    assert_eq!(
        candidates(&bare),
        candidates(&aux_engine_with_input("kaifa"))
    );
    bare.push_aux_code('k');
    let query = bare.query().unwrap();
    assert!(query.candidates.items.is_empty());
    assert_eq!(query.marked_text(), "kai'fa;k");
}

/// 同一段拼音在纯拼音态下的候选，给上面的对比用。
fn aux_engine_with_input(input: &str) -> Engine {
    let mut engine = aux_engine();
    engine.set_input(input);
    engine
}

/// 触发键校验：单字符、ASCII 可打印、非字母数字、非翻页键。
#[test]
fn trigger_key_validation() {
    assert!(is_valid_aux_code_key(';'));
    assert!(is_valid_aux_code_key('\''));
    assert!(!is_valid_aux_code_key('a'));
    assert!(!is_valid_aux_code_key('1'));
    assert!(!is_valid_aux_code_key('['));
    assert!(!is_valid_aux_code_key('.'));
    assert!(!is_valid_aux_code_key('中'));
    let mut engine = aux_engine();
    assert_eq!(engine.aux_code_key(), ';');
    engine.set_aux_code_key('a');
    assert_eq!(engine.aux_code_key(), ';');
    engine.set_aux_code_key('/');
    assert_eq!(engine.aux_code_key(), '/');
    engine.set_input("kaifa");
    assert!(engine.aux_trigger('/'));
    assert!(!engine.aux_trigger(';'));
