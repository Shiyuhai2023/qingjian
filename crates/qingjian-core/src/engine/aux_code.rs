...
/// 触发键校验：单字符、ASCII 可打印、不是字母数字、不是翻页键。设置页与配置加载共用。
pub fn is_valid_aux_code_key(key: char) -> bool {
    key.is_ascii_graphic() && !key.is_ascii_alphanumeric() && !PAGING_KEYS.contains(&key)
}

impl Engine {
    /// 敲下 \`key\` 是不是要进辅码态。四条边界：
    ///
