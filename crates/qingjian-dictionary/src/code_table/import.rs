...
use super::imported::CodeTableImport;
...
pub fn import_code_table(
    source: &Path,
    dest_dir: &Path,
) -> Result<CodeTableImport, DictionaryError> {
    ...
    let (table, mut metadata, report) = if Container::is_qj(source) {
        let table = CodeTable::open(source)?;
        let metadata = table.metadata().cloned().unwrap_or_default();
        let report = CodeTableImportReport {
            read: table.len(),
            with_code: table.len(),
            ..CodeTableImportReport::default()
        };
        ...
        let table = CodeTable::from_pairs(parsed.pairs)?;
...
fn read_all(source: &Path) -> Result<(ParsedTable, CodeTableImportReport), DictionaryError> {
    let mut merged = ParsedTable::default();
    let mut report = CodeTableImportReport::default();
